...
{
    public class SubscriptionPlanChangingConfiguration : IEntityTypeConfiguration<SubscriptionPlanChanging>
    {
        #region Configure 
        public void Configure(EntityTypeBuilder<SubscriptionPlanChanging> builder)
        {
            builder.ToTableName("RosasSubscriptionPlanChanges");
            builder.HasKey(x => x.Id);
            builder.HasOne(b => b.Plan).WithMany(p => p.SubscriptionPlanChanges).HasForeignKey(f => f.PlanId).OnDelete(DeleteBehavior.Restrict);
            builder.HasOne(b => b.PlanPrice).WithMany(p => p.SubscriptionPlanChanges).HasForeignKey(f => f.PlanPriceId).OnDelete(DeleteBehavior.Restrict);
