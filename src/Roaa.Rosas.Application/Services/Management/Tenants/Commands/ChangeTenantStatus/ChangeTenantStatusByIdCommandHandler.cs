...
public class ChangeTenantStatusByIdCommandHandler : IRequestHandler<ChangeTenantStatusByIdCommand, Result<List<TenantStatusChangedResultDto>>>
{
    ...
    {
        return await _tenantService.SetTenantNextStatusAsync(request.TenantId, request.Status, request.ProductId, request.Action, null, request.Notes, cancellationToken);
    }
