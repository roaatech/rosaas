...
namespace Roaa.Rosas.Application.Payment.Platforms.StripeService
{
    ...
    {
        Task<Result<CompleteSessionResultModel>> CompleteSuccessfulSessionPaymentAsync(string sessionId, Guid orderId, CancellationToken cancellationToken = default);

        Task<Result<CompleteSessionResultModel>> CompleteFailedSessionPaymentAsync(string sessionId, Guid orderId, CancellationToken cancellationToken = default);
